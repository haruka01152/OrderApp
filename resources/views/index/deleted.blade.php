...
@include('components.header')
    ...
    <form id="restoreForm" class="max-w-7xl lg:container m-auto w-8/12 bg-white overflow-hidden shadow py-8 px-16" action="{{route('restore',$construction->id)}}" method="post">
            ...
            <div class="py-7 text-center block fixed -inset-x-0 -bottom-0 z-10" style="background:rgba(0,0,0,.2);">
                <input id="restoreSubmit" type="button" onclick="submit();" value="復元" class="bg-yellow-600 text-white text-xl rounded-lg py-2 px-8 cursor-pointer">
            </div>
        </form>
    </div>
...
<script>
    $('#restoreForm').find('input, textarea, select').not('#restoreSubmit, input:hidden').prop('disabled', true);
</script>